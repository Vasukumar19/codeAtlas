...
from app.enrichment.pipeline import KnowledgeContext
...
async def test_embedding_orchestrator():
    ...
    # Process again (Cache should hit because we use Mock DB returning empty set in fallback, wait, our fallback returns empty set!)
    # Let's mock the cache properly for the test to ensure it bypasses
    # We will inject the hash into the cache mock
